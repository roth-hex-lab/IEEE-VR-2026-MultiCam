...
#ifndef M3T_INCLUDE_M3T_AZURE_KINECT_Multi2_H_
#define M3T_INCLUDE_M3T_AZURE_KINECT_Multi2_H_
 ...
 */
class AzureKinectMulti2 {
 public:
  // Singleton instance getter
  static AzureKinectMulti2 &GetInstance();
  AzureKinectMulti2(const AzureKinectMulti2 &) = delete;
  AzureKinectMulti2 &operator=(const AzureKinectMulti2 &) = delete;
  ~AzureKinectMulti2();
  ...
  bool SetUp(uint32_t device_index);
 ...
 */
class AzureKinectColorMulti2 : public ColorCamera {
 public:
  // Constructors, destructor, and setup method
  AzureKinectColorMulti2(uint32_t device_index, const std::string &name,
                        float image_scale = 1.05f,
                         bool use_depth_as_world_frame = false);
  AzureKinectColorMulti2(uint32_t device_index, const std::string &name,
                         const std::filesystem::path &metafile_path);
  AzureKinectColorMulti2(const AzureKinectColorMulti2 &) = delete;
  AzureKinectColorMulti2 &operator=(const AzureKinectColorMulti2 &) = delete;
  ~AzureKinectColorMulti2();
  bool SetUp();
 ...
 private:
  ...
  // Data
  uint32_t device_index_;
  AzureKinectMulti2 &azure_kinect_;
 ...
 */
class AzureKinectDepthMulti2 : public DepthCamera {
 public:
  // Constructors, destructor, and setup method
  AzureKinectDepthMulti2(uint32_t device_index, const std::string &name,
                        float image_scale = 1.0f,
                         bool use_color_as_world_frame = true,
                         float depth_offset = 0.0f);
  AzureKinectDepthMulti2(uint32_t device_index, const std::string &name,
                         const std::filesystem::path &metafile_path);
  AzureKinectDepthMulti2(const AzureKinectDepthMulti2 &) = delete;
  AzureKinectDepthMulti2 &operator=(const AzureKinectDepthMulti2 &) = delete;
  ~AzureKinectDepthMulti2();
  bool SetUp() override;
