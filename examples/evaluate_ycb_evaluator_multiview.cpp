...
#include "evaluate_ycb_evaluator_multiview.h"
...
void global_scene_fusion(
  ...
  //body2world_poses_final = body2world_poses_all[0];
  m3t::Transform3fA obj_pose, cam_pose;
  for (int detect_id = view_ids.size()-1; detect_id > -1; detect_id--) {
    //std::vector<std::vector<float>> center_point;
    //center_point.clear();
    //for (int cam_id = body2world_poses_all.size() - 1; cam_id > -1; cam_id--) {
      //auto coord = body2world_poses_all[cam_id][obj_id].translation();
    if (TWC.find(view_ids[detect_id]) != TWC.end()) {
      ...
      body2world_poses_map[obj_ids[detect_id]] = cam_pose*obj_pose;
        //center_point.push_back({coord(0, 0), coord(1, 0), coord(2, 0)});
        //body2world_poses_final[obj_id] = body2world_poses_all[cam_id][obj_id];
      //}
    }
    /*
    if (center_point.size() > 0) {
      auto average = toEigenMatrix(center_point).colwise().mean();
      // std::cout << average;
      body2world_poses_final[obj_id].translation() = average;
    }*/
  }
}
...
void YCBEvaluator::SaveCamPoses(const Result &result, const std::string &title) {
  std::ofstream ofs;
  std::filesystem::path result_campose_path_{
      dataset_directory_ / "result" /
      ("scenertm6dcamview" + std::to_string(num_view_) + "before_ycbv-test.csv")};
...
void YCBEvaluator::SavePoses(const Result &result, const std::string &title) {
  std::filesystem::path result_objpose_path_ {
    dataset_directory_ / "result" / ("scenertm6dobjview" +
        std::to_string(num_view_) +"_ycbv-test.csv")};
  std::ofstream ofs;
  ...
  std::filesystem::path result_campose_path_{dataset_directory_ / "result" /
                                             ("scenertm6dcamview" +
        std::to_string(num_view_) + "after_ycbv-test.csv")};
...
bool YCBEvaluator::EvaluateRunConfiguration(
  ...
  */
    ...
    std::map<int, m3t::Transform3fA> body2world_poses_map;
    global_scene_fusion(candidates, body2world_poses_map, TWC);
...
bool YCBEvaluator::SetUpTracker(
  ...
  *tracker_ptr = std::make_shared<m3t::Tracker>("tracker");
  ...
  std::filesystem::path color_camera_directory{
      dataset_directory_ / "test" / run_configuration.sequence_name / "rgb"};
  std::filesystem::path depth_camera_directory{
      dataset_directory_ / "test" / run_configuration.sequence_name / "depth"};
...
void YCBEvaluator::GetBodyPoses(
    const std::shared_ptr<m3t::Tracker> &tracker_ptr,
    std::vector<m3t::Transform3fA> &body2world_poses) {
    ...
    for (int update_iteration = 0;
      ...
      for (int cam_id = 1; cam_id < color_camera_ptrs.size(); cam_id++) {
        ...
        for (int obj_id = 0; obj_id < body_name2idx_map_.size(); obj_id++) {
          ...
          if (!body_ptrs[obj_id]->body2world_pose().matrix().isZero(0)) {
...
void YCBEvaluator::UpdateCameras(std::vector<int> load_indices) const {
  for (int cam_id = 0; cam_id < num_view_; ++cam_id) {
...
void YCBEvaluator::Posedetector(
...
/*
int detectInliersRansac(const Eigen::Matrix3Xf &ref_xyz,
                                       const Eigen::Matrix3Xf &target_xyz,
                                       const Eigen::Matrix4Xf &ref_xyzw,
                                       const Eigen::Matrix4Xf &target_xyzw,
                        std::vector<char> *inliers,
                        std::vector<int> &inlier_indices_) {
  int min_inliers_ = 10;
  double point_error_threshold_ = 0.02;
  int sac_iterations_ = 700;
  ...
  Eigen::Matrix3f sampled_ref_xyz, sampled_target_xyz;
  int num_matches = ref_xyz.cols();
  for (int i = 0; i < sac_iterations_; ++i) {
    // Randomly select 3 different matches
    int a = random_int(0, num_matches);
    int b = a;
    while (a == b) {
      b = random_int(0, num_matches);
    }
    int c = a;
    while (c == a || c == b) {
      c = random_int(0, num_matches);
    }
    ...
    Eigen::Matrix4f estimate =
        Eigen::umeyama(sampled_ref_xyz, sampled_target_xyz);

    // find inliers based on reprojection
    // bot_tictoc("match scoring");
    
    Eigen::VectorXf point_err =
        (estimate * ref_xyzw - target_xyzw).colwise().norm();
    //std::cout << "debug" << std::endl;
    //std::cout << num_matches << std::endl;
    //std::cout << point_err.size()<< std::endl;
    double loss = 0;
    for (int m_ind = 0; m_ind < num_matches; ++m_ind) {
      if (m_ind == a || m_ind == b || m_ind == c) {
        continue;
      }
      if (point_err(m_ind) > point_error_threshold_) {
        loss += point_error_threshold_;
      }  // else do nothing
      if (loss > min_loss) {
...
/*
 bool Ransacunmeyama(const Eigen::Matrix4Xf &ref_xyzw,
                    const Eigen::Matrix4Xf &target_xyzw,
                    std::vector<char> *inliers, Eigen::Matrix4f &ume_estimate) {
  int min_inliers_ = 10;
  std::vector<int> inlier_indices_;
  Eigen::Matrix3Xf ref_xyz =
      ref_xyzw.topRows<3>().cwiseQuotient(ref_xyzw.row(3).replicate<3, 1>());
  Eigen::Matrix3Xf target_xyz = target_xyzw.topRows<3>().cwiseQuotient(
      target_xyzw.row(3).replicate<3, 1>());

  int num_matches = ref_xyz.cols();
  int num_inliers =
      detectInliersRansac(ref_xyz, target_xyz, ref_xyzw, target_xyzw, inliers, inlier_indices_);
  if (num_inliers < min_inliers_) {
    return false;
  }

  // estimate rigid body transform with inliers
  Eigen::Matrix4Xf inlier_ref_xyzw(4, num_inliers);
  Eigen::Matrix4Xf inlier_target_xyzw(4, num_inliers);
  Eigen::Matrix3Xf inlier_ref_xyz(3, num_inliers);
  Eigen::Matrix3Xf inlier_target_xyz(3, num_inliers);
  ...
  ume_estimate =
      Eigen::umeyama(inlier_ref_xyz, inlier_target_xyz);
  return true;
 }
 */

/*
void YCBEvaluator::calculate_cam_poses(
    std::vector<m3t::Transform3fA> body2world_main,
                         std::vector<m3t::Transform3fA> body2world_sub,
                         Eigen::Matrix4f &dTransRT, float &error) {
  Eigen::Matrix4f cam_0, cam_1;
  int num_pcld = 0;
  std::vector<std::vector<float>> pcld_sub;
  std::vector<std::vector<float>> pcld_main;
  std::vector<std::vector<float>> pclds;
  std::vector<float> point3f;
  Eigen::Matrix<float, 4, Eigen::Dynamic> pcld_obj_main(4, kpts_gt_[0].size());
  Eigen::Matrix<float, 4, Eigen::Dynamic> pcld_obj_sub(4, kpts_gt_[0].size());
  //std::vector<int> symmetry_ids = {0, 12, 13, 15, 17, 18, 19, 20};
  for (int pose_idx = 0; pose_idx < body2world_main.size(); pose_idx++) {
    if (!body2world_main[pose_idx].matrix().isZero(0) &&
        body2world_main[pose_idx].matrix().array().isFinite().all() &&
        !body2world_main[pose_idx].matrix().isZero(0) &&
        body2world_sub[pose_idx].matrix().array().isFinite().all()) {
      cam_0 = body2world_main[pose_idx].matrix();
      cam_1 = body2world_sub[pose_idx].matrix();
      if (std::count(symmetry_ids.begin(), symmetry_ids.end(), pose_idx) > 0) {
        num_pcld += 1;
        point3f = {cam_0(0, 3), cam_0(1, 3), cam_0(2, 3), 1};
        pcld_main.push_back(point3f);

        point3f = {cam_1(0, 3), cam_1(1, 3), cam_1(2, 3), 1};
        pcld_sub.push_back(point3f);
      } else {
        pclds.clear();
        for (int i = 0; i < kpts_gt_[pose_idx].size(); i++) {
          num_pcld += 1;
          pclds.push_back({kpts_gt_[pose_idx][i].x, kpts_gt_[pose_idx][i].y,
                           kpts_gt_[pose_idx][i].z, 1});
        }
        pcld_obj_main = cam_0 * toEigenMatrix(pclds).transpose();
        pcld_obj_sub = cam_1 * toEigenMatrix(pclds).transpose();
        for (int i = 0; i < pcld_obj_main.cols(); i++) {
          pcld_main.push_back({pcld_obj_main(0, i), pcld_obj_main(1, i),
                               pcld_obj_main(2, i), 1});
          pcld_sub.push_back(
              {pcld_obj_sub(0, i), pcld_obj_sub(1, i), pcld_obj_sub(2, i), 1});
        }
      }
    }
  }
  if (num_pcld > kpts_gt_[0].size()) {
    Eigen::Matrix<float, 3, Eigen::Dynamic> cloud_tgt(3, num_pcld);
    cloud_tgt = toEigenMatrix(pcld_main).block(0, 0, num_pcld, 3).transpose();
    Eigen::Matrix<float, 3, Eigen::Dynamic> cloud_src(3, num_pcld);
    cloud_src = toEigenMatrix(pcld_sub).block(0, 0, num_pcld, 3).transpose();
    //bool status;
    //std::vector<char> inliers;
    // using unmeyama with ransac
    //status = Ransacunmeyama(toEigenMatrix(pcld_sub).transpose(), toEigenMatrix(pcld_main).transpose(), &inliers, dTransRT);
    
    //if (status == false)
    // if ransac fails then use umeyama
    dTransRT = Eigen::umeyama(cloud_src, cloud_tgt, false);
    // std::cout << "target" << std::endl;
    // std::cout << cloud_src << std::endl;
    // std::cout << "result" << std::endl;
    // std::cout << dTransRT << std::endl;
    error = std::sqrt((dTransRT * toEigenMatrix(pcld_sub).transpose() -
                       toEigenMatrix(pcld_main).transpose())
                          .block(0, 0, 3, num_pcld)
                          .colwise()
                          .squaredNorm()
                          .mean());
  } else {
    dTransRT = Eigen::Matrix4f::Identity();
    error = 1;
  }
}
*/
...
bool YCBEvaluator::LoadGTPoses(
  ...
  std::filesystem::path obj_path{dataset_directory_ / "test" /sequence_name /
                             "scene_gt.json"};
  std::filesystem::path cam_path{dataset_directory_ / "test" / sequence_name /
                                 "scene_camera.json"};
  // Open file
